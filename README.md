...
Responsive Design: Fully responsive layout built with Tailwind CSS, accessible on desktop and mobile.

Frosted Glass UI: Content sections are semi-transparent with a backdrop-blur effect, allowing the animation to show through.

Project Showcase: A grid layout detailing key projects like EdiGenAI, AUREXA-SE, and Sentinel Project.
...
Tailwind CSS: For all styling and layout (utility-first CSS framework).

JavaScript (ES6+): To initialize and configure the background animation.
...
index.html: Contains all HTML, CSS (via Tailwind CDN and <style> tags), and JavaScript (in <script> tags).
